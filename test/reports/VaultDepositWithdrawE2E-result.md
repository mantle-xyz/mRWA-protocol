...
| MockUSDC | `0xc40fa5d8cf408baa63019137033d2698377fb243` |
...
| 0g | User USDC balance | 3037960000 |
...
|---|--------|--------|---------|--------|
| A1 | Mint 1000 USDC to User | `mint(User, 1000000000)` | `0xf1b095bb8003a2724258b00688d3ec0b36f50dfa4f62c32ee06b998884c2865c` | ✓ User USDC: 4037960000 |
| A2 | Approve **Vault** | `approve(Vault, 1000000000)` | `0x7093c8d42145511af1f46fd50eee364afd59906490fa116040e686baa0477fca` | ✓ |
| A3 | Deposit 1000 USDC via Gateway | `gateway.deposit(1000000000)` | `0x15cc7897645bbcc26d5423454dd5e71bea3b02c43e36b8c2e330e8f8c5f4d678` | ✓ shares minted: 1000000000 |
| A4 | Verify shares | — | — | 1000000000 ✓ |
| A5 | Verify vault USDC | — | — | 1000000000 ✓ |
...
| A10 | Sync redeem 490686766 shares via Gateway | `gateway.redeem(490686766)` | `0xb4b265f3798232701d6d587ed7a0d430fd60b9f98bca8bee6c1e0dad7bd7a363` | ✓ 500000000 USDC received |
...
| A14 | User USDC | — | — | 3537960000 (increased by 500000000) ✓ |
...
- Rate at 1.02e18: each share worth 1.02 USDC
- Withdrew 500 USDC: previewWithdraw → 490686766 shares (accounts for 0.1% redemption fee)
- Gross USDC = 490686766 × 1.02e18 / 1e18 = 500500021 → fee = 500021 → net ≈ 500000000 ✓
...
| # | Action | Params | Tx Hash | Result |
|---|--------|--------|---------|--------|
| B1 | Mint 1000 USDC to User | `mint(User, 1000000000)` | `0x6faab256d93f92fac84d9bb4af3108e02d0195aee436fede14539f8af01f6698` | ✓ |
| B2a | Approve vault | `approve(Vault, 1000000000)` | `0xb1635aee47efcb77be62a1ea24dbdf003aa514d2505e92632bd5785df300aa38` | ✓ |
| B2b | Deposit 1000 USDC | `gateway.deposit(1000000000)` | `0xd4b8ba38b65958eb3f46d4131dc70945e3647ad66a58135204d068a6c08e226b` | ✓ shares: 980392156 (at 1.02e18) |
...
| B16 | User USDC | — | — | 4047450000 (increased by 509490000) ✓ |
| B16b | User shares | — | — | 989705390 (decreased by 500000000) ✓ |

### Path B Analysis

- Deposit at 1.02e18: 1000e6 USDC → 980392156 shares (1000e6 × 1e18 / 1.02e18)
...
- No separate claim step — markRequestsDone transfers USDC ✓
...
| # | Action | Params | Tx Hash | Result |
|---|--------|--------|---------|--------|
| C1 | Mint 1000 USDC to User | `mint(User, 1000000000)` | `0xa29c7fea4d388efbed4f8ae0bcbaee9fda1588190a8274d290fffeb553177864` | ✓ |
| C2a | Approve vault | `approve(Vault, 1000000000)` | `0x5865cf9debba3aafd636e239540169e0eb4e8bf962c986e7cb478f49599b502a` | ✓ |
| C2b | Deposit 1000 USDC | `gateway.deposit(1000000000)` | `0x76aaea6e3a43cb9abb3afea4de8092ebaee2e8a90ec4235f2c0766fa176f8545` | ✓ shares: 980392156 (at 1.02e18) |
| C3 | Rebalance (invest) | `rebalance()` | `0x17230731b1368df6b825f8a84facaffc7a3c0ddeb46a49c1779bcad6387d694a` | ✓ InvestExecuted emitted, USDC → adapter → SubRed |
| C4 | MockSubRedManagement USDC | — | — | received invest amount ✓ |
...
| C18a | settleRedeem (off-chain) | _triggered by off-chain service_ | — | ✓ USDC sent to adapter |
| C18b | Adapter USDC balance | — | — | 1990509700 ✓ |
| C18c | settleAdapter (sweep USDC + confirm redeem) | `settleAdapter(adapter, 0, 1990509700, [], [2])` | `0xd23191a82205f42d30f2e1d31f72f3a3c0eb2e0745f9160de5287f14b9a7aae5` | ✓ USDC swept to vault |
| C18d | Vault USDC / redeemInFlight | — | — | vault USDC=1990509700, redeemInFlight=0, freeCash=1481019700 ✓ |
...
| C21 | Rebalance after finalize | `rebalance()` | `0x51f2f23b3aaf0d7b18e77b8c2f8b241730f7fe04607826021ada81d7aebc2803` | ✓ remaining USDC re-invested |
| C22 | User USDC | — | — | 4556940000 (increased by 509490000) ✓ |
| C23 | User shares | — | — | 1470097546 (decreased by 500000000) ✓ |

### Path C Analysis

- **Invest flow**: `rebalance()` → `adapter.deposit()` → `SubRed.subscribe()` → USDC leaves vault. Off-chain `settleSubscribe` mints 181602600e18 ST tokens to adapter. `settleAdapter` sweeps ST to vault and confirms investInFlight.
- **Divest flow**: `processRedeemBatch()` → `adapter.requestRedeemAsync()` → `SubRed.redeem()` → ST tokens leave vault. Off-chain `settleRedeem` sends 1990509700 USDC to adapter. `settleAdapter` sweeps USDC to vault and confirms redeemInFlight.
- **Settlement amounts**: Invested 1990510000 USDC, received back 1990509700 USDC (300 USDC difference — rounding/fee from SubRed mock).
- **ST token pricing**: adapter `totalValue()` returned 181602600 (USDC equivalent), indicating the ST token oracle/pricing is configured. The minted 181602600e18 ST tokens correspond to ~181.6 USDC value at the oracle price, less than the invested 1990510000 USDC — indicating the mock settlement used a different conversion ratio.
- Rebalance after finalize re-invested remaining vault USDC into the adapter (freeCash → 0).
...
| 2 | USDC approval must target Vault, not Gateway | **Critical** | Fixed A2: approve Vault address |
