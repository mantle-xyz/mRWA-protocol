...
contract GatewaySyncRedeemQATest is Test {
    ...
    function test_SyncRedeem_Normal() public {
        ...
        uint256 expectedGross = DEPOSIT_AMOUNT; // rate=1e18 => 1:1
        uint256 expectedFee = expectedGross * FEE_BPS / BPS_DENOMINATOR; // 10e6
        uint256 expectedNet = expectedGross - expectedFee; // 990e6
    ...
    function test_SyncRedeem_InsufficientFreeCash() public {
        ...
        mockAccountant.setExchangeRate(targetRate);
        ...
        vm.prank(userA);
        vm.expectRevert(abi.encodeWithSignature("ERC4626ExceededMaxRedeem(address,uint256,uint256)", userA, shares, maxRedeemable));
        gateway.redeem(shares);
    ...
    function test_SyncRedeem_SanctionedUserRoutesToSafe() public {
        _logCase(
            ...
            unicode"被制裁用户同步赎回会将 shares 路由给到 safe 地址，并触发对应事件"
        );
    ...
    function test_SyncRedeem_ZeroFee() public {
        ...
        uint256 shares = vault.balanceOf(userA);
        uint256 expectedAssets = DEPOSIT_AMOUNT; // rate=1e18, fee=0 => 1:1
