...
contract AsyncRedeemFairnessQATest is Test {
    ...
    function setUp() public {
        ...
        // OperatorExecutor — UUPS, init now (independent of controller address)
    ...
    function test_OperatorCanSkipEarlyRequests_QueueJumping() public {
        ...
        _processRedeemBatch(_singleId(reqLate));
    ...
    function test_InsufficientFunds_WholeBatchReverts() public {
        ...
        uint256 totalRequired = overAmount * 2;
        _step(string.concat("  settle total = ", vm.toString(totalRequired), " > vault balance"));

        _step("[Step 3] Attempt finalizeRedeemBatch, expect Vault__InsufficientPhysicalCash revert");
        // The revert fires on the second iteration (i=1) after the first transfer already executed
        // within the same call frame, so the re-read balance is vaultBal - overAmount.
        uint256 expectedPhysicalCash = vaultBal - overAmount;
    ...
    function test_EarlyAndLateBatch_ExchangeRateDifference() public {
        ...
        _step("[Step 2] Change exchange rate to 1.1e18 (10% appreciation) via AccountantExecutor");
        _updateExchangeRate(1.1e18);
        ...
        _processRedeemBatch(_singleId(reqEarly));
        _finalizeRedeemBatch(_singleId(reqEarly), _singleAmount(estEarly));

        _step("[Step 5] Process and settle late batch");
        _processRedeemBatch(_singleId(reqLate));
        _finalizeRedeemBatch(_singleId(reqLate), _singleAmount(estLate));
    ...
    function test_BatchProcessing_PendingAndLockedDecrement() public {
        ...
        (,,,,uint256 est1,,,) = vault.requests(req1);
        _finalizeRedeemBatch(_singleId(req1), _singleAmount(est1));
        ...
        (,,,,uint256 est2,,,) = vault.requests(req2);
        _finalizeRedeemBatch(_singleId(req2), _singleAmount(est2));
    ...
    function test_SameOwner_MultiBatch_TotalSettledCorrect() public {
        ...
        _finalizeRedeemBatch(batch2Ids, batch2Settled);
    ...
    function test_SmallRequestProcessedFirst_LargeStays() public {
        ...
        _processRedeemBatch(_singleId(reqSmall));
        _finalizeRedeemBatch(_singleId(reqSmall), _singleAmount(estSmall));
