...
contract GatewayAsyncRedeemQATest is Test {
    ...
    function test_AsyncRedeem_MinThreshold() public {
        _logCase(
            ...
            unicode"最小异步赎回门槛可阻止垃圾小额请求进入队列"
        );

        _step(string.concat("[Step 1] minRedeemAmount: ", vm.toString(MIN_REDEEM)));

        // Construct tiny shares that result in estimated assets below minRedeemAmount.
        // With rate=1e18 and fee=1%, need net < 100e6 => gross < ~101.01e6 => shares < ~101.01e6
        // Use 50e6 shares => gross = 50e6, fee ~= 0.5e6, net ~= 49.5e6 < 100e6
        uint256 tinyShares = 50e6;
        _step(string.concat("[Step 2] Attempt requestRedeem with tiny shares: ", vm.toString(tinyShares)));

        vm.prank(userA);
        vm.expectPartialRevert(IMantleYieldVault.Vault__BelowMinRedeem.selector);
    ...
    function test_AsyncRedeem_BelowMinRedeem() public {
        ...
        // Small shares: 10e6 => gross=10e6, fee=0.1e6(ceil), net < 100e6
        uint256 tinyShares = 10e6;
        uint256 gross = tinyShares; // rate=1e18 => 1:1
        uint256 fee = (gross * FEE_BPS + BPS_DENOMINATOR - 1) / BPS_DENOMINATOR;
        uint256 estimatedNet = gross - fee;
        _step(string.concat("[Step 2] tinyShares: ", vm.toString(tinyShares)));
        _step(string.concat("[Step 2] estimated net: ", vm.toString(estimatedNet)));
        assertTrue(estimatedNet < MIN_REDEEM, "estimated net should be below minRedeemAmount");

        vm.prank(userA);
        vm.expectRevert(abi.encodeWithSelector(IMantleYieldVault.Vault__BelowMinRedeem.selector, estimatedNet, MIN_REDEEM));
        gateway.requestRedeem(tinyShares);
    ...
    function test_AsyncRedeem_EstimatedAssetsIsReference() public {
        _logCase(
            ...
            unicode"estimatedAssets 仅为请求创建时的参考值，汇率变动后 settledAssets 可以不同"
        );
        ...
        _step(string.concat("  new rate = ", vm.toString(mockAccountant.exchangeRate())));

        // Compute settled amount based on new rate (same as operator would)
        (,, uint256 netShares,,,,, ) = vault.requests(requestId);
        uint256 settledAmount = netShares * 1.05e18 / 1e18;
        ...
        assertTrue(settledAmount != estimatedAssets, "settled should differ from estimated due to rate change");
