...
contract MockStrategyAdapter_EL is IStrategyAdapter {
    ...
    function setRevertTotalValue(bool r) external { revertTotalValue = r; }
    ...
    function getPosTokenPrice() external view returns (uint256) {
        if (revertTotalValue) revert("PRICE_FAIL");
        return 1e18;
    }
    ...
    function deposit(uint256 amount, address) external returns (uint256) {
        ...
        MockPosToken_EL(POS_TOKEN).mint(address(this), amount);
        return amount;
    }
...
contract ExtremeLiquidityQATest is Test {
    ...
    function setUp() public {
        ...
        adapter = new MockStrategyAdapter_EL(address(usdc), address(posToken));
        adapter.setVault(address(vault));
        adapter2 = new MockStrategyAdapter_EL(address(usdc), address(posToken));
    ...
    function test_AsyncStrategyDelayed_BatchCannotFinalize() public {
        ...
        vm.prank(bot);
        vm.expectRevert(
            abi.encodeWithSelector(StrategyController.InsufficientCashForReady.selector, estAssets, vaultBal)
        );
    ...
    function test_RedeemInFlight_ConfirmRecordsActualValue() public {
        ...
        // Lower buffer so rebalance triggers divest from over-allocated adapter
    ...
    function test_MultiStress_CoreViewsConsistent() public {
        ...
        _step("[Step 5] Apply multi-stress: adapter value crash + more locked shares");
        // Simulate adapter value crash by burning posToken (market event)
        uint256 adapter1Val = adapter.totalValue();
        uint256 adapter2Val = adapter2.totalValue();
        if (adapter1Val > 1_000e6) posToken.burn(address(adapter), adapter1Val - 1_000e6);
        if (adapter2Val > 1_000e6) posToken.burn(address(adapter2), adapter2Val - 1_000e6);
    ...
    function test_BankRun_TotalPayoutNotExceedPhysicalBalance() public {
        ...
        uint256 totalDepositedUsdc = 40_000e6; // 4 users * 10000e6
    ...
    function test_LowUSDC_HighRedeemInFlight_AssetsVsPayability() public {
        ...
        // Partial redeem at maxRedeem should succeed (if maxR > 0)
        if (maxR > 0) {
            vm.prank(user1);
            ...
            _step("  maxRedeem=0, all sync redeem blocked");
        }
    ...
    function test_MultiAdapterUnpaid_DepositStillWorks() public {
        ...
        uint256[] memory settled = new uint256[](1);
        if (vaultBalNow >= est) {
            // Vault has enough (new deposits provided liquidity), finalize succeeds
            settled[0] = est;
            vm.prank(bot);
            executor.executeFinalizeRedeemBatch(address(controller), ids, settled);
            ...
            // Vault lacks USDC (adapters still unpaid), finalize reverts
            settled[0] = est;
            vm.prank(bot);
