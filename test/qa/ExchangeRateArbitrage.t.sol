...
contract MockSyncSettlementVenue_ERA {
    MockUSDC_ERA public immutable ASSET;
    MockPosToken_ERA public immutable POS_TOKEN;

    constructor(address asset_, address posToken_) {
        ASSET = MockUSDC_ERA(asset_);
        POS_TOKEN = MockPosToken_ERA(posToken_);
    }
...
contract MockStrategyAdapter_ERA is IStrategyAdapter {
    ...
    MockSyncSettlementVenue_ERA public immutable SETTLEMENT_VENUE;
    address public vaultAddr;

    constructor(address asset_, address posToken_, address settlementVenue_) {
        ASSET = asset_;
        POS_TOKEN = posToken_;
        SETTLEMENT_VENUE = MockSyncSettlementVenue_ERA(settlementVenue_);
    }
...
contract ExchangeRateArbitrageQATest is Test {
    ...
    MockSyncSettlementVenue_ERA internal settlementVenue;
    ...
    function setUp() public {
        ...
        // Deploy AccountantExecutor for Rule 3 compliance: bot -> executor -> accountant
        ...
        settlementVenue = new MockSyncSettlementVenue_ERA(address(usdc), address(posToken));
        adapter = new MockStrategyAdapter_ERA(address(usdc), address(posToken), address(settlementVenue));
    ...
    /// @dev Warp and update exchange rate via bot -> AccountantExecutor -> Accountant.
    function _updateRate(uint64 newRate) internal {
        uint64 ts = uint64(block.timestamp);
        vm.prank(bot);
        acctExecutor.executeUpdateRate(address(accountant), newRate, ts);
    }
    ...
    /// @dev Seed the accountant's totalSharesLastSettle via bot -> AccountantExecutor.
    ...
    function test_ContinuousDepositsAcrossRateChanges() public {
        ...
        uint256 expectedSharesA1 = 1000e6 * 1e18 / uint256(INITIAL_RATE);
        assertEq(sharesA1, expectedSharesA1, "shares at rate 1.0 should match depositAmt * 1e18 / rate");
    ...
    function test_AsyncRedeemBeforeRateDecrease() public {
        ...
        (,, uint256 netShares,, uint256 estimatedAssets,,,) = vault.requests(reqId);
        _step(string.concat("  estimatedAssets (at rate 1.1): ", vm.toString(estimatedAssets)));

        _step("[Step 3] Rate drops to 1.0e18");
        uint64 newRate = 1.0e18;
        _warpAndUpdateRate(1, newRate);
        ...
        // Compute settlement from net shares at the new (lower) rate
        uint256 reducedSettle = netShares * uint256(newRate) / 1e18;
    ...
    function test_AsyncRedeemBeforeRateIncrease() public {
        ...
        uint64 newRate = 1.1e18;
        _warpAndUpdateRate(1, newRate);

        _step("[Step 4] Finalize with settledAssets > estimatedAssets (rate rose, operator settles more)");
        // Compute settlement from net shares at the new (higher) rate
        uint256 higherSettled = netShares * uint256(newRate) / 1e18;
    ...
    function test_StaleRateContinuousDeposits() public {
        ...
        // Verify using contract formula: grossAssets = shares * rate / 1e18, fee = ceil(gross * feeBps / BPS)
        uint256 grossA = sharesA * 1.1e18 / 1e18;
        uint256 feeA = (grossA * FEE_BPS + BPS - 1) / BPS;
        uint256 expectedValueA = grossA - feeA;
        assertEq(valueA, expectedValueA, "value should match previewRedeem formula at new rate");
        assertEq(valueA, valueB, "equal shares yield equal value");
        assertEq(valueA, valueC, "equal shares yield equal value");
    ...
    function test_StaleRateContinuousRedemptions() public {
        ...
        (,, uint256 netSharesC,, uint256 estimatedAssetsC,,,) = vault.requests(reqId);
        ...
        uint64 newRateC = 0.9e18;
        _warpAndUpdateRate(1, newRateC);
        ...
        vm.prank(bot);
        opExecutor.executeProcessRedeemBatch(address(controller), ids);
        // Finalize with reduced settlement computed from net shares at the new (lower) rate
        uint256 reducedSettle = netSharesC * uint256(newRateC) / 1e18;
        uint256[] memory settled = new uint256[](1);
        settled[0] = reducedSettle;
        vm.prank(bot);
        opExecutor.executeFinalizeRedeemBatch(address(controller), ids, settled);
    ...
    // -----------------------------------------------------------------------
    // 10. Management fee settlement and rate update happen separately
    // -----------------------------------------------------------------------
    function test_ManagementFeeSettleWithRateUpdate() public {
        _logCase(
            "test_ManagementFeeSettleWithRateUpdate",
            unicode"管理费结算与汇率更新分别发生时，对前后用户的份额价值影响"
        );

        _step("[Step 1] userA deposits at rate 1.0e18");
        _depositForUser(userA, 10_000e6);

        _step("[Step 2] Seed fee base by calling settleManagementFee (sets totalSharesLastSettle)");
        _seedFeeBase();

        _step("[Step 3] Warp 180 days to accrue management fee");
        vm.warp(block.timestamp + 180 days);
        uint256 totalSupplyBefore = vault.totalSupply();
        _step(string.concat("  totalSupply before settle: ", vm.toString(totalSupplyBefore)));

        _step("[Step 4] Call settleManagementFee independently via executor");
        vm.prank(bot);
        acctExecutor.executeSettleManagementFee(address(accountant));
        uint256 totalSupplyAfterSettle = vault.totalSupply();
        uint256 feeShares = totalSupplyAfterSettle - totalSupplyBefore;
        _step(string.concat("  totalSupply after settle: ", vm.toString(totalSupplyAfterSettle)));
        ...
        _step("[Step 5] Update rate to 1.05e18 (separate from fee settlement)");
        _updateRate(1.05e18);

        _step("[Step 6] userB deposits after fee settlement + rate update");
    ...
    function test_DepositThenAsyncRedeemAroundRateIncrease() public {
        ...
        _depositForUser(userA, depositAmount);
        uint256 shares = vault.balanceOf(userA);
        ...
        assertGt(estimatedAssets, depositAmount, "estimated assets should exceed original deposit (arbitrage profit)");
        _step("  [Finding] Short-cycle arbitrage window exists: deposit at low rate, redeem at high rate");

        _step("[Step 4] Process and finalize settlement");
        uint256[] memory ids = new uint256[](1);
        ids[0] = reqId;
        // Rate increase caused cash deficit — add liquidity so processRedeemBatch succeeds
        uint256 deficit = vault.getCashDeficit();
        if (deficit > 0) _addVaultLiquidity(deficit);
        vm.prank(bot);
        opExecutor.executeProcessRedeemBatch(address(controller), ids);
        uint256[] memory settled = new uint256[](1);
        ...
        // userA started with userABalanceBefore, spent depositAmount on deposit, got back ~1089e6
        assertGt(userABalance, userABalanceBefore, "userA should have more USDC than started (arbitrage profit)");
        _step(string.concat("  profit: ", vm.toString(userABalance - userABalanceBefore)));
    ...
    function test_DepositThenSyncRedeemAroundRateIncrease() public {
        ...
        // fee = ceil(1080e6 * 1%) = 10.8e6 (exact, Ceil rounding)
        // net = 1080e6 - 10.8e6 = 1069.2e6, profit = 69.2e6
    ...
    function test_SyncRedeemFailFallbackToAsync() public {
        ...
        // Sync redeem drained cash — add liquidity to cover remaining deficit so process succeeds
        uint256 deficit = vault.getCashDeficit();
        if (deficit > 0) _addVaultLiquidity(deficit);
    ...
    function test_CircuitBreakerBlocksWriteOps() public {
        ...
        vm.warp(block.timestamp + 1);
        vm.prank(bot);
        acctExecutor.executeUpdateRate(address(accountant), 1.2e18, uint64(block.timestamp));
        assertTrue(accountant.paused(), "Accountant should be paused after circuit breaker");
        _step("  Accountant is now paused");

        assertEq(accountant.lastExchangeRate(), INITIAL_RATE, "Rate should remain at old value");
        _step("  lastExchangeRate unchanged");
    ...
    function test_CircuitBreakerRecoveryViaEmergency() public {
        ...
        uint64 correctedRate = 1.005e18;
        vm.prank(admin);
        accountant.emergencyRateUpdate(correctedRate);
        assertFalse(accountant.paused(), "should be unpaused after emergency update");
        assertEq(accountant.lastExchangeRate(), correctedRate, "rate should be corrected");
    ...
    function test_BotOverLimitUpdateEventsCoexist() public {
        ...
        _step("[Step 3] Verify CircuitBreakerTriggered event emitted");
        bytes32 cbSig = keccak256("CircuitBreakerTriggered(uint256,uint256,uint256)");
        bool foundCB = false;
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].topics[0] == cbSig) { foundCB = true; break; }
        }
        assertTrue(foundCB, "CircuitBreakerTriggered event should be emitted");

        _step("[Step 4] Verify accountant state");
    ...
    function test_AsyncRedeemRateUpdateOrderMatters() public {
        ...
        assertGt(estA, estB, "estA should be > estB (more shares, requested at lower rate)");
    ...
    // -----------------------------------------------------------------------
    // 18. Fee settlement and rate write happen separately: user ops differ before/after
    // -----------------------------------------------------------------------
    function test_FeeSettleBeforeRateWrite() public {
        _logCase(
            "test_FeeSettleBeforeRateWrite",
            unicode"管理费结算与汇率更新分开调用，验证用户在操作前后的差异"
        );
        ...
        _step("[Step 4] userB deposits before fee settlement and rate update");
        _depositForUser(userB, 10_000e6);
        uint256 sharesB = vault.balanceOf(userB);
        _step(string.concat("  userB shares (before update): ", vm.toString(sharesB)));

        _step("[Step 5] Call settleManagementFee independently via executor");
        uint256 supplyBefore = vault.totalSupply();
        vm.prank(bot);
        acctExecutor.executeSettleManagementFee(address(accountant));
        uint256 supplyAfterSettle = vault.totalSupply();
        uint256 feeShares = supplyAfterSettle - supplyBefore;
        _step(string.concat("  fee shares minted: ", vm.toString(feeShares)));
        assertGt(feeShares, 0, "should mint fee shares");

        _step("[Step 6] Update rate to 1.05e18 (separate from fee settlement)");
        _updateRate(1.05e18);

        _step("[Step 7] userC deposits after fee settle + rate update");
    ...
    function test_RateUpdateFailureNoHalfState() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            Accountant.Accountant__StaleComputeTimestamp.selector, lastCompute, lastCompute
        ));
        vm.prank(bot);
        acctExecutor.executeUpdateRate(address(accountant), 1.05e18, lastCompute); // same as last -> stale
        ...
        vm.warp(block.timestamp + 1);
        vm.prank(bot);
        acctExecutor.executeUpdateRate(address(accountant), 1.5e18, uint64(block.timestamp)); // 50% deviation
        assertTrue(accountant.paused(), "should be paused");
    ...
    function test_ProcessBatchBeforeAfterRateUpdate() public {
        ...
        assertGt(batchTotalAsset1, 0, "precondition: batch 1 event must have been captured");
        ...
        // Rate increase caused cash deficit — add liquidity so processRedeemBatch succeeds
        {
            uint256 deficit2 = vault.getCashDeficit();
            if (deficit2 > 0) _addVaultLiquidity(deficit2);
        }
        ...
        assertGt(batchTotalAsset2, 0, "precondition: batch 2 event must have been captured");
    ...
    function test_FeeSettleTreasurySharesAffectRatio() public {
        _logCase(
            ...
            unicode"结费导致 treasury 获得新 shares 后，再次赎回会改变剩余用户占比"
        );
        ...
        _step("[Step 3] Warp 180 days, trigger fee settlement via executor");
        vm.warp(block.timestamp + 180 days);
        vm.prank(bot);
        acctExecutor.executeSettleManagementFee(address(accountant));
        ...
        assertGt(treasuryMaxRedeem, 0, "precondition: treasury should have redeemable shares from fee");
        vm.prank(treasury);
    ...
    function test_CircuitBreakerEmergencyRecoveryFullChain() public {
        ...
        assertEq(oldRate, INITIAL_RATE, "rate unchanged");
        ...
        uint64 emergencyRate = 1.005e18;
        vm.prank(admin);
        accountant.emergencyRateUpdate(emergencyRate);
        assertFalse(accountant.paused(), "should be unpaused");
        assertEq(accountant.lastExchangeRate(), emergencyRate, "rate updated");
