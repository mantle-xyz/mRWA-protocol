...
contract AdapterRegistryQATest is Test {
    ...
    function test_RemoveAdapter_WithInFlightRejected() public {
        ...
        _step("[Step 2] Create an in-flight record for adapterA");
        // Deposit USDC to vault so createInFlight can proceed
        _depositToVault(makeAddr("depositor"), 1000e6);
        // Approve adapter to receive USDC from vault
        vm.prank(controller);
        vault.approveToAdapter(adapterA, address(usdc), 1000e6);
        // Create invest in-flight: USDC going out, waiting for tokens
        vm.prank(controller);
        vault.createInFlight(adapterA, address(usdc), 1000e6, 1000e6, true);
        _step(string.concat("  adapterInvestInFlightTokens: ", vm.toString(vault.adapterInvestInFlightTokens(adapterA))));
        _step("  PASS: in-flight created");

        _step("[Step 3] Try to remove adapterA with in-flight");
        vm.prank(controller);
        vm.expectRevert(abi.encodeWithSelector(IMantleYieldVault.Vault__AdapterHasInFlight.selector, adapterA));
