...
contract MockSanctionsOracle_GovRisk is ISanctionsOracle {
    ...
    function isSanctioned(address account) external view override returns (bool) { return sanctioned[account]; }
    function isWhitelisted(address account) external view override returns (bool) { return whitelisted[account]; }
    function setSanctioned(address account, bool status) external { sanctioned[account] = status; }
    function setWhitelisted(address account, bool status) external { whitelisted[account] = status; }
...
contract GovernanceRiskQATest is Test {
    ...
    function test_AdminSwapCoreComponents() public {
        ...
        // Step 6: Old controller fails
        _step("[Step 6] Verify old controller calls are rejected");
        uint256[] memory emptyIds = new uint256[](0);
        vm.prank(address(controller));
        vm.expectRevert(IMantleYieldVault.Vault__OnlyController.selector);
        vault.updateRequestBatch(emptyIds, IMantleYieldVault.RequestStatus.PROCESSING);
        _step("  Old controller call correctly reverts with Vault__OnlyController");

        // Step 7: New controller can operate
        _step("[Step 7] Verify new controller can operate");
        vm.prank(address(newController));
        vault.updateRequestBatch(emptyIds, IMantleYieldVault.RequestStatus.PROCESSING);
        _step("  New controller can call updateRequestBatch");
