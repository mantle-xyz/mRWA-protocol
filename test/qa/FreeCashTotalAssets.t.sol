...
contract MockStrategyAdapter is IStrategyAdapter {
    ...
    function deposit(uint256 amount, address) external returns (uint256) {
        ...
        MockPosToken(POS_TOKEN).mint(address(this), amount);
        return amount;
    }

    function withdrawSync(uint256 amount, address) external returns (uint256) {
        // Burn posToken (redeem from underlying) and return USDC to vault
        MockPosToken(POS_TOKEN).burn(address(this), amount);
...
contract FreeCashTotalAssetsQATest is Test {
    ...
    function setUp() public {
        ...
        adapter = new MockStrategyAdapter(address(usdc), address(posToken));
    ...
    function test_FreeCash_ZeroWhenLockedEqualsBalance() public {
        _logCase(
            ...
            unicode"rebalance 投资后请求赎回 + 汇率上升，使 floatingLocked 恰好等于物理余额，freeCash=0"
        );
    ...
    function test_SyncRedeem_LimitedByFreeCash() public {
        ...
        _step("[Step 2] User1 requests async redeem of all shares, locking most freeCash");
    ...
    // -----------------------------------------------------------------------
    // 6. test_RedemptionFeeChange_AffectsNetShares
    // -----------------------------------------------------------------------
    ...
    function test_RedemptionFeeChange_AffectsNetShares() public {
        ...
        uint256 shares2 = _depositViaGateway(user2, 3000e6);
    ...
    function test_AdapterTotalValue_RevertPropagates() public {
        ...
        _step("[Step 3] Set adapter to revert on getPosTokenPrice(), expect totalAssets to revert");
        adapter.setRevertTotalValue(true);

        vm.expectRevert("PRICE_FAIL");
        vault.totalAssets();
