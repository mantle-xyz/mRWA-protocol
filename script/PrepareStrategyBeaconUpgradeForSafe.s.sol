...
contract PrepareStrategyBeaconUpgradeForSafeScript is Script {
    function run() external {
        TimelockUpgradeController timelock =
            TimelockUpgradeController(payable(vm.envAddress("TIMELOCK_ADDRESS")));
        ...
        if (executeOnchain) {
            ...
            uint256 callerPk = vm.envOr(
                "UPGRADE_CALLER_PRIVATE_KEY", vm.envOr("DEPLOYER_PRIVATE_KEY", vm.envUint("PRIVATE_KEY"))
            );
